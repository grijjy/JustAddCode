...
type
  ...
  private
    FBar: ISmartPtr<TBar>;
  public
    property Bar: ISmartPtr<TBar> read FBar write FBar;
  end;
  ...
  private
    FFoo: ISmartPtr<TFoo>;
  public
    property Foo: ISmartPtr<TFoo> read FFoo write FFoo;
  end;
...
begin
  ...
  Foo := TSmartPtr<TFoo>.Create(TFoo.Create);
  Bar := TSmartPtr<TBar>.Create(TBar.Create);
  ...
  { This will result in a memory leak since the smart pointers reference
    ...
    get a message saying that 4 objects have been leaked: TFoo, TBar,
    TSmartPtr<TFoo> and TSmartPtr<TBar>. }
